  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {Object.entries(data.services).map(([name, svc]) => (
          ...
          >
            ...
            {svc.details && <p className="text-xs text-red-400 mt-1">{svc.details}</p>}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
        ...
        />
        <MetricCard label="Transactions (5m)" value={data.platform.recentTransactions5m} variant="info" />
        <MetricCard label="Calls (1h)" value={data.platform.recentCalls1h} />
        <MetricCard
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        >
          ...
          <p
            className={`text-2xl font-bold ${
              data.platform.negativeBalanceUsers === 0 ? 'text-emerald-400' : 'text-red-400'
            }`}
          >
        ...
        <div className="border border-gray-800 rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-white">{data.platform.balanceDiscrepancies}</p>
          <p className="text-xs text-gray-500 mt-1">Checks credit − debit vs actual balance</p>
        </div>
      </div>
      ...
      <p className="text-[10px] text-gray-600 text-right">Server time: {data.serverTime}</p>
    </div>
  );
};
